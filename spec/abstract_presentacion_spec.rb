...
describe 'AbstractPresentation class' do
  it 'could be initialized without parameters' do
    @ap=AbstractPresentation.new
    expect(@ap.html_with_keywords).to eq ""
  end

  it '.html_with_keywords should add br on newlines' do
    @text="a\nb"
    #@keywords=%w{key1 key2}
    @ap=AbstractPresentation.new(@text)
    expect(@ap.html_with_keywords).to eq("a<br/>b")
  end
  it ".html_with_keywords should add <strong> on keywords, by default" do
    @text="key1 key2 key3"
    @keywords=%w{key1 key2}
    @ap=AbstractPresentation.new(@text)
  ...
  it ".html_with_keywords should replace strong with another tag" do
    @text="key1 key2 key3"
    @keywords=%w{key1 key2}
    @ap=AbstractPresentation.new(@text)
    @ap.keywords =@keywords
  ...
  it ".html_with_keywords should add class to tag" do
    @text="key1 key2 key3"
    @keywords=%w{key1 key2}
    @ap=AbstractPresentation.new(@text)
    @ap.keywords =@keywords
