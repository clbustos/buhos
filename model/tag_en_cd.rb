class Tag_En_Cd < Sequel::Model
  ...
  def self.cds_rs_tag(revision,tag)
    $db["SELECT cd.*,SUM(IF(decision='yes',1,0)) n_pos, SUM(IF(decision='no',1,0))  n_neg FROM tags_en_cds tcd INNER JOIN canonicos_documentos cd ON tcd.canonico_documento_id=cd.id WHERE tcd.tag_id=? AND tcd.revision_sistematica_id=? GROUP BY canonico_documento_id HAVING n_pos>0", tag.id,revision.id]
    end
